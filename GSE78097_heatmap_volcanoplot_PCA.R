...
# --------------------------
cat("Step 1: Loading and preprocessing GSE78097 data...\n")

# Load GEO series matrix file (no platform info)
gse_file <- 'd:/dualdisease/T1D/P.1/GEO.data3/GSE78097_series_matrix.txt'
...
anno_file <- "d:/dualdisease/T1D/P.1/GEO.data3/GSE78097.top.table.tsv"
...
# Save merged dataset
output_file <- "GSE78097_expr&group.txt"
...
pdf("PCA3D_DEG_GSE78097.pdf", width = 7, height = 6)
scatter3D(
  ...
  main = "3D PCA - GSE78097 (T1D vs Healthy)"
)
# Add legend
...
cat("✓ 3D PCA plot saved as PCA3D_DEG_GSE78097.pdf\n")
...
pdf("volcano_GSE78097.pdf", width = 10, height = 8)
volcano_plot <- ggplot(vol_data, aes(x = logFC, y = log10FDR, color = group)) +
  ...
  labs(x = "log2 Fold Change", 
       ...
       title = "Volcano Plot - GSE78097 (T1D vs Healthy)",
       color = "Expression") +
  theme_classic() +
  theme(
...
cat("✓ Volcano plot saved as volcano_GSE78097.pdf\n")
...
pdf("pheatmap_GSE78097.pdf", width = 10, height = 8)
pheatmap(
  heat_scaled,
  color = heat_palette,
  show_rownames = FALSE,
  ...
  border_color = NA,
  fontsize = 12,
  main = "Differentially Expressed Genes - GSE78097 (T1D vs Healthy)",
  treeheight_row = 20,
  treeheight_col = 20,
...
cat("✓ Heatmap saved as pheatmap_GSE78097.pdf\n")
...
# Save DEG list
write.table(deg, "GSE78097_DEGs.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save volcano plot data
write.table(vol_data, "GSE78097_volcano_data.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save normalized heatmap data
write.table(heat_scaled, "GSE78097_heatmap_data.txt", sep = "\t", quote = FALSE)

cat("\n=============================================\n")
cat("Analysis completed successfully!\n")
cat("Output files:\n")
cat("  - GSE78097_expr&group.txt (merged expression and clinical data)\n")
cat("  - GSE78097_DEGs.txt (differentially expressed genes)\n")
cat("  - PCA3D_DEG_GSE78097.pdf (3D PCA visualization)\n")
cat("  - volcano_GSE78097.pdf (volcano plot)\n")
cat("  - pheatmap_GSE78097.pdf (DEG heatmap)\n")
cat("=============================================\n")