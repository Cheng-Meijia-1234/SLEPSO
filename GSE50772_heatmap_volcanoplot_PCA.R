...
library('dplyr')
library('tidyverse')
setwd("d:/dualdisease/SLE/SLE.1/")

# Retrieve data from GEO database using specified filename without obtaining platform information
gse <- getGEO(filename='d:/dualdisease/SLE/SLE.1/GEO.data/GSE50772_series_matrix.txt',getGPL=F)
...
library(tidyverse)
gset <- read_tsv("d:/dualdisease/SLE/SLE.1/GEO.data/GSE50772.top.table.tsv")
gset[1:4,1:6]
dim(gset)
...
# Merge "ann" and "expr" data frames by ID (keep all rows from both data frames)
expr2 <- merge(ann, expr, by = "ID")  
expr2[1:4,1:6]
...
clin$`disease status:ch1`
table(clin$`disease status:ch1`)

# Extract ID and disease status information
mc <- clin[,c("ID","disease status:ch1")]
mc[1:4,]
names(mc)[2] <- "group"
table(mc$group)
dim(mc)
...
df[1:4,1:4]
table(df$group)
# expr3$Gene.symbol=trimws(str_split(expr3$Gene.symbol,'//',simplify = T)[,1])
...
# Save merged data
write.table(data.frame(df),file = "GSE50772_expr&group.txt", sep = "\t",quote = F,row.names = T)
...
# Plot 3D PCA
pdf("PCA3D_DEG_GSE50772_NEW.pdf",width = 5,height = 5)
...
expr2 <- expr2[!duplicated(expr2$Gene.symbol),]
...
# Select key columns
expr2_com <- expr2_combined[,c(2:4,86)]
colnames(expr2_com)
...
# Output files
write.table(ig,file = "GSE50772_expression.txt",sep = "\t",row.names=T,col.names = T)
write.table(updown,file = "GSE50772_expression_updown.txt",sep = "\t",row.names=T,col.names = T)
...
# Levels specified as "up", "down", and "ns"
dt_f$group <- factor(dt_f$group,
                     levels = c("up","down","ns"),
                     ordered = T)

# Start plotting - create mapping
# Create scatter plot object p1 using ggplot2 package
p1 <- ggplot(data=dt_f,aes(logFC,log10FDR,color=group))+
  geom_point(size=1.6)

# Set manual color scale with transparency
...
mytheme<-theme_classic()+
  theme(text=element_text(family = "sans",colour ="gray30",size = 12),  # Set font style
        axis.line = element_line(size = 0.6,colour = "gray30"),         # Control axis line appearance
        axis.ticks = element_line(size = 0.6,colour = "gray30"),        # Control tick mark appearance
        axis.ticks.length = unit(1.5,units = "mm"),                     # Control tick mark length (mm)
        plot.margin=unit(x=c(top.mar,right.mar,bottom.mar,left.mar),    # Control plot margins (inches)
                         units="inches"))

# Apply custom theme and save volcano plot
pdf("volcano_GSE50772.pdf",width = 6,height = 5)
p4+mytheme  +geom_hline(yintercept = c(-log10(0.05)),  # Add horizontal dashed line for significance threshold (0.05)
                        linewidth = 0.7,
                        color = "black",
                        lty = "dashed")+
  geom_vline(xintercept = c(-0.585,0.585),  # Add vertical dashed lines for fold change thresholds
             linewidth = 0.7,
             color = "black",
             lty = "dashed")
dev.off()
...
table(dfGroup$Group)
t_id <- row.names(filter(dfGroup,`Group`=='SLE'))

# Get sample IDs for control group
n_id <- row.names(filter(dfGroup,`Group`=='Control'))
...
ann <- data.frame(Type=factor(rep(c('SLE','Control'),c(61,20))),
                  row.names = colnames(geo.expr1))

# Define colors for SLE and Control groups
ann_colors <- list(Type = c("SLE" = "#b83db8", "Control" = "#75af6c")) #F2A2AF

# Save heatmap to PDF
pdf("pheatmap_GSE50772.pdf",width = 6,height = 5)
...
nrow(pre_heatdata)#[1]2373

# Save up/down regulated gene lists
deg_SLE1 <- as.data.frame(dt_f) 
deg_SLE1[1:4,]
deg_SLE1 <- deg_SLE1[,c("group","Gene.symbol")]
table(deg_SLE1$group)

# Extract up and down regulated gene symbols
deg_SLE1_up <- deg_SLE1[deg_SLE1$group == "up",]$Gene.symbol
deg_SLE1_down <- deg_SLE1[deg_SLE1$group == "down",]$Gene.symbol

# Save gene lists to RData file
save(deg_SLE1_up, deg_SLE1_down, file = "DEG_SLE1_up_down.RData")
load("DEG_SLE1_up_down.RData")